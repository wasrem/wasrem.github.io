...
function NavBar() {
  ...
  return (
    <>
      <div className="bg-white shadow-md">
        <div className="container mx-auto px-4">
          <div className="flex justify-between items-center pt-4 pb-2">
            <div className="flex items-center space-x-4">
              <div id="bookmark-container" className="flex items-center space-x-2">
                ...
                <button className="font-bold text-gray-600 hover:text-gray-800 transition duration-300" onClick={handleBookmark} id="bookmark">
                  즐겨찾기
                </button>
              </div>
            </div>
            ...
            <ul className="hidden md:flex space-x-8">
              {["/", "Cafe Link", "Blog Link"].map((link, index) => (
                <li key={index}>
                  <a
                    href={link}
              ...
              <li>
                <button className="font-bold text-gray-600 hover:text-blue-800 hover:border-b-2 hover:border-blue-800 transition duration-300" onClick={toggleContact}>
                  Contact
                </button>
              </li>
            </ul>
          ...
          <div className="flex justify-between items-center py-4">
            <button className="text-gray-500 hover:text-gray-700 focus:outline-none transition duration-300 ease-in-out transform hover:scale-150" onClick={toggleSideMenu}>
              <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 6h16M4 12h16M4 18h16"></path>
              </svg>
            </button>

            <Link to="/" className="text-2xl font-bold text-gray-800 hover:text-gray-700 transition duration-300 mx-auto">
        ...
        <div
          className={`fixed inset-0 bg-black bg-opacity-50 z-40 transition-opacity duration-300 ease-in-out ${isSideMenuOpen ? "opacity-100" : "opacity-0 pointer-events-none"}`}
        ...
        <div
          ...
          <div className="p-6">
            ...
            <ul className="flex flex-col space-y-6 mt-8">
              {["/", "Cafe Link", "Blog Link"].map((link, index) => (
                <li key={index}>
                  <a href={link} rel="noopener noreferrer" target="_blank" className="font-bold text-gray-600 hover:text-gray-800 transition duration-300" onClick={toggleSideMenu}>
                    {index === 0 ? "Home" : link}
                  </a>
                </li>
              ))}
      ...
      {isContactOpen && (
        <>
          <div className="fixed inset-0 bg-black bg-opacity-50 z-40 transition-opacity duration-300 ease-in-out" onClick={toggleContact}></div>
          <div
            id="contact-element"
            ...
            <div
              ...
              <ul className="space-y-4">
                {myContactList.map((item, index) => (
                  <li key={index}>
                    <a href={item.href} rel="noopener noreferrer" target="_blank" className="text-black hover:border-b-2 hover:border-white transition duration-300">
                      {item.text}
                    </a>
                  </li>
                ))}
